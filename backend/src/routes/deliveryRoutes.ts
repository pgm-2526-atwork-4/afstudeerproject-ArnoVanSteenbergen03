    ...
    try {
      ...
      const deliveriesWithChannels = await Promise.all(
        openDeliveries.map(async (delivery) => ({
          ...delivery,
          supplierChannel: await getSupplierChannelForDelivery(
            delivery.activity.providerId,
          ),
        })),
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const driverAssignmentCondition =
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (activity.assignedDriver || activity.status !== "requested") {
        return res
          .status(400)
          .json({ error: "Delivery has already been accepted" });
      }
      ...
      if (!updatedActivity) {
        return res
          .status(409)
          .json({ error: "Delivery was accepted by another driver" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (activity.status !== "need_assistance") {
        return res
          .status(400)
          .json({ error: "This delivery does not need assistance" });
      }
...
// status transitions for deliveries
